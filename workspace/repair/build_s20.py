...
old='<div class="bsband"><span class="n">5 briefs</span><span class="lbl">Board-Style &middot; Next-Step Questions</span><p class="t">Infectious Disease</p></div>'
edits.append({"scope":"page","op":"replace","old":old,"new":old.replace('5 briefs','6 briefs'),"why":"band count"})
m=re.search(r'<h3 class="system" id="id">.*?</h3>',src);assert '+ 4 board-style' in m.group(0)
edits.append({"scope":"page","op":"replace","old":m.group(0),"new":m.group(0).replace('+ 4 board-style','+ 5 board-style'),"why":"header count"})
old='data-item-id="q_1a0084c4964253578d96" data-item-version="1"'
edits.append({"scope":"brief:neonatal-rash","op":"replace","old":old,"new":old.replace('version="1"','version="2"'),"why":"stem age corrected 2 week to 2 day (meaning change)"})
REV={'herpangina':' Part II <b>Viral Exanthems in a Child</b> sorts hand-foot-and-mouth disease among the other rashes with fever.',
     'bs-fever-rash-arthralgia':' Part II <b>Viral Exanthems in a Child</b> owns the rash-with-fever differential without joint pain, including varicella postexposure care.'}
for b_,txt in REV.items():
    m=re.search(r'<div class="pearls"><span class="lbl">Pairs with</span>.*?</div>',BR[b_],re.S);assert m and BR[b_].count(m.group(0))==1,b_
    edits.append({"scope":"brief:"+b_,"op":"replace","old":m.group(0),"new":m.group(0)[:-6]+txt+'</div>',"why":"reverse link"})
json.dump({"pass":"s20","date":"2026-09-23","reason":"UWorld batch: exanthems brief, 4 backfills.","expected":{},"edits":edits},
          open('repair/LEDGER-s20.json','w'),indent=1,ensure_ascii=False)

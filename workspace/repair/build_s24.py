...
for f in ['s24_a','s24_b']:
    for o in load(f):
        ...
        for b,lis in by.items():
            edits.append({"scope":"brief:"+b,"op":"insert_before","anchor":"</ol>","text":''.join(lis),"why":"s24 backfill items"})
json.dump({"pass":"s24","date":"2026-09-23","reason":"s24: Meckel (occult-gi-bleed) and Ewing/bone tumor library (bone-tumors) backfills.","expected":{},"edits":edits},
          open('repair/LEDGER-s24.json','w'),indent=1,ensure_ascii=False)

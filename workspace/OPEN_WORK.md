...
- Open: `bs-puv` and `bs-leukemia` keep the NBME badge; their source questions are UWorld, so their "Aquifer" subtitle credit needs the user's call. `aq-bruising` source-case block is a placeholder (`data-recon="partial"`) until the case narrative is supplied. Page-wide: `.rows` grids (auto 1fr) squeeze multi-word labels when the right column is long.

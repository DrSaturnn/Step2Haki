...
4. Draft. Backfills into an existing brief: worker packet (`python3 repair/efficiency/make_packet.py <T> <brief> <source.md> "<Q heading>" "<task>" index.html <out.md>`), one agent per brief, agent runs the packet's `tools/verify_edits.py <edits> --nids <source.md>` from the workspace folder. New briefs and Aquifer workups: full spec (`repair/workup/SPEC_s19.md`, `SPEC_aq.md`) until piloted.
